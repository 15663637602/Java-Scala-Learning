...
public class QuickSort {
    ...
    public static void quickSort(int[] arr){
        ...
        quickSorts(0, arr.length - 1, arr);
    }

    private static void quickSorts(int l, int h, int[] arr){
        if(l > h) return;
        int low = l;
        int high = h;
        int p = arr[low];
        ...
        arr[low] = p;
        quickSorts(l, low - 1, arr);
        quickSorts(low + 1, h, arr);
    }
    ...
    private static void sort(int a[], int l, int h) {
        ...
        sort(a, l, low - 1); // 对低子表进行递归排序
        sort(a, low + 1, h); // 对高子表进行递归排序
